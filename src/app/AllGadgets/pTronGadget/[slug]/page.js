...
import { AllpTronGadgetsList } from '@/GadgetData/AllpTronGadgetsList';

export async function generateMetadata({ params }) {
  const resolvedParams = await params;
  const slug = resolvedParams.slug;
  const GadgetSEO = AllpTronGadgetsList.find(item => item.slug === slug);
...
export default async function Page({ params }) {
  ...
  const resolvedParams = await params;
  const slug = resolvedParams.slug;
  const GadgetDetails = AllpTronGadgetsList.find(item => item.slug === slug);

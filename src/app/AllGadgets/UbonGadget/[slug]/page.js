...
import { AllUbonGadgetsList } from '@/GadgetData/AllUbonGadgetsList';

export async function generateMetadata({ params }) {
  const resolvedParams = await params;
  const slug = resolvedParams.slug;
  const GadgetSEO = AllUbonGadgetsList.find(item => item.slug === slug);
...
export default async function Page({ params }) {
  ...
  const resolvedParams = await params;
  const slug = resolvedParams.slug;
  const GadgetDetails = AllUbonGadgetsList.find(item => item.slug === slug);
